...
```sh title:"Find MS-RPRN spooler endpoints with valid creds"
...
```sh title:"Find MS-RPRN spooler endpoints anonymously"
...
```sh title:"Scan for WebClient service hosts (WebDAV coercion targets)"
...
```sh title:"PrinterBug - coerce spooler to attacker listener"
...
```sh title:"PetitPotam MS-EFSRPC coercion to attacker listener"
...
```sh title:"Dementor alternate spooler coerce"
...
```sh title:"Relay coerced DC$ auth to LDAPS, add computer + delegate-access"
...
```sh title:"S4U impersonate target user via RBCD-controlled computer"
...
```sh title:"DCSync over Kerberos using ccache from coerce chain"
...
```sh title:"PrintNightmare driver-load DLL drop (real SMB share required)"
