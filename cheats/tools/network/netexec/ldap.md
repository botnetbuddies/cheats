...
```sh title:"NetExec LDAP Full collection over LDAP, target DC as DNS resolver"
...
```sh title:"NetExec LDAP Export to users.txt for spraying/roasting"
...
```sh title:"NetExec LDAP Read ReadGMSAPassword-protected blobs via LDAP"
...
```sh title:"NetExec LDAP Execute a custom filter when canned modules don't fit"
...
```sh title:"NetExec LDAP Spray userlist with empty password, no creds required"
...
```sh title:"NetExec LDAP Query LDAP for DONT_REQ_PREAUTH accounts with valid creds"
...
```sh title:"NetExec LDAP Map internal hosts without port scanning (get-network module)"
...
```sh title:"NetExec LDAP Read ms-Mcs-AdmPwd cleartext from readable computer objects"
...
```sh title:"NetExec LDAP Check ms-DS-MachineAccountQuota (RBCD prereq if >0)"
...
```sh title:"NetExec LDAP Read domain DACL for Get-Changes / Get-Changes-All"
...
```sh title:"NetExec LDAP Request TGS for every SPN-enabled account, save to file"
...
```sh title:"NetExec LDAP List unconstrained/constrained delegation principals"
...
```sh title:"NetExec LDAP Write config pointed at target DC, move to /etc/"
