...
```sh title:"Aggressive all-port TCP, default scripts, OS guess"
...
```sh title:"All-port TCP only, no scripts, fastest first sweep"
...
```sh title:"Ping sweep host discovery"
nmap -sn $ip_range
```
<!-- cheat
var ip_range
...
```sh title:"Scan targets from file"
...
```sh title:"Scan top 100 TCP ports"
...
```sh title:"Scan top 5000 TCP ports"
...
```sh title:"Scan specific TCP ports"
...
```sh title:"TCP connect scan through proxychains"
...
```sh title:"Top 100 UDP ports, fast UDP triage"
...
```sh title:"Slow SYN scan with capped rate"
...
```sh title:"Fast full TCP sweep with masscan"
...
```sh title:"Run -sC -sV after a port list, enrich findings"
...
```sh title:"HTTP NSE on 80/443/8080/8443"
...
```sh title:"SMB NSE on 139/445, signing/CVEs/share list"
...
```sh title:"FTP NSE on 21 (anon/banner/bounce)"
...
```sh title:"DNS NSE: SD, NSID, attempt AXFR zone transfer"
...
```sh title:"MySQL NSE: info/dbs/users/variables/audit"
...
```sh title:"MSSQL NSE: info/config/empty-password check"
...
```sh title:"RDP NSE: encryption + NTLM info + MS12-020"
...
```sh title:"LDAP NSE on 389/636 (root DSE, search, novell, MS-DS)"
...
```sh title:"Wide unauth sweep of LDAP+SMB AD ports"
...
```sh title:"Kerberos user enum via pre-auth errors, no creds"
