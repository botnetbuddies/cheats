...
```sh title:"Uv Install impacket suite (secretsdump, psexec, etc) from fortra fork"
...
```sh title:"Uv Install NetExec (nxc), network pentesting swiss army knife"
...
```sh title:"Uv Install BloodHound.py collector for BloodHound Community Edition"
...
```sh title:"Uv Install bloodyAD, Active Directory privesc framework (LDAP/SAMR)"
...
```sh title:"Uv Install git-dumper, extracts .git directories from exposed web servers"
...
```sh title:"Uv Install Certipy, AD Certificate Services (ADCS) enum and abuse"
...
```sh title:"Uv Install Penelope shell handler, auto upgrades reverse shells to PTY"
...
```sh title:"Uv Install Responder, LLMNR/NBT-NS/MDNS poisoner for hash capture"
...
```sh title:"Uv Install powerview.py, interactive AD enumeration/abuse over LDAP"
