...
```sh title:"BloodyAD dMSA migration takeover, omit the trailing dollar sign"
...
```sh title:"BloodyAD Grant Get-Changes + Get-Changes-All on domain root"
...
```sh title:"BloodyAD Make target Kerberoastable by writing an SPN"
...
```sh title:"BloodyAD Grant WRITE_DACL on DN for later ACL takeover"
...
```sh title:"BloodyAD Enough for SPN write or shadow creds without full control"
...
```sh title:"BloodyAD groupMember adds principal to group"
...
```sh title:"BloodyAD Full control, reset password, add members, rewrite ACL"
...
```sh title:"BloodyAD UAC flag required for S4U2Self / protocol transition"
...
```sh title:"BloodyAD Set DONT_REQ_PREAUTH to make target ASREPRoastable"
...
```sh title:"BloodyAD delegate_from can impersonate any user to delegate_to"
...
```sh title:"BloodyAD Needs MachineAccountQuota > 0 for RBCD / shadow creds"
...
```sh title:"BloodyAD ADIDNS entry for WPAD / MITM / coercion targets"
...
```sh title:"BloodyAD Map inbound/outbound trusts across the forest"
...
```sh title:"BloodyAD Export every ADIDNS record (adidnsdump equivalent)"
...
```sh title:"BloodyAD Built-in collector, fallback when bloodhound-python fails"
...
```sh title:"BloodyAD Read msDS-AllowedToActOnBehalfOfOtherIdentity"
...
```sh title:"BloodyAD Dump every readable attribute on an object"
...
```sh title:"BloodyAD Read msDS-ManagedPassword (auto-decoded to NT hash)"
...
```sh title:"BloodyAD Raw blob for manual decode via gMSADumper"
...
```sh title:"BloodyAD Map every object you can write to (ACL privesc paths)"
...
```sh title:"BloodyAD Read single attribute from object (targeted lookup)"
...
```sh title:"BloodyAD Force reset, requires Reset Password right or GenericAll"
...
```sh title:"BloodyAD WriteOwner abuse, take object ownership"
...
```sh title:"BloodyAD UPN swap for ESC9/ESC10 certificate abuse"
...
```sh title:"BloodyAD Change mail attribute, password reset / phishing prep"
...
```sh title:"BloodyAD ESC14b explicit X509 mapping via altSecurityIdentities"
...
```sh title:"BloodyAD Destructive SPN overwrite (use add spn to append instead)"
...
```sh title:"BloodyAD Remove time-window restriction on account authentication"
...
```sh title:"BloodyAD Reset UAC to 512 NORMAL_ACCOUNT, clears custom flags"
...
```sh title:"BloodyAD Cheap write to confirm access without side effects"
...
```sh title:"BloodyAD Write description, admins sometimes stash creds here"
...
```sh title:"BloodyAD Write msDS-AllowedToDelegateTo for S4U2Proxy targets"
...
```sh title:"BloodyAD Change ms-DS-MachineAccountQuota (default 10)"
...
```sh title:"BloodyAD Restore deleted object by DN, SID, or unique name"
...
```sh title:"BloodyAD Clear ACCOUNTDISABLE bit to re-enable stale account"
...
```sh title:"BloodyAD Drop membership, cleanup after privesc"
...
```sh title:"BloodyAD Clean up attacker-created computer account"
...
```sh title:"BloodyAD Undo AllowedToActOnBehalfOfOtherIdentity entry"
...
```sh title:"BloodyAD Targeted cleanup after forced Kerberoast"
...
```sh title:"BloodyAD Run raw LDAP filter when subcommands don't fit"
...
```sh title:"BloodyAD Include deleted objects via show-deleted + show-recycled OIDs"
